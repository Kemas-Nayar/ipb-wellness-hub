...
const IconAlert = () => (
  <svg width="40" height="40" viewBox="0 0 24 24" fill="none"
    stroke="#C8102E" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"
    aria-hidden="true">
...
const canCheckIn = (status) => status === 'upcoming' || status === 'berlangsung';
...
const RiwayatReservasiPage = ({ onNavigate, onBack, fromPage = 'profile', user, refreshTrigger }) => {
  ...
  const fetchPage = useCallback(async (pageNum = 1, isRefresh = false) => {
    ...
    const { data, error: fetchError, count } = await supabase
      .from('reservations')
      .select('id, date, start_time, end_time, gym_name, notes, created_at', { count: 'exact' })
      .eq('user_id', user.id)
  ...
  return (
    <div className="riwayat-page">
      ...
      <div className="riwayat-header">
        ...
        <button
          className="riwayat-refresh-btn"
          onClick={() => fetchPage(1, true)}
          aria-label="Muat ulang daftar reservasi"
          disabled={refreshing}
        >
      ...
      {loading ? (
        ...
        <div className="riwayat-center">
          ...
          <button className="riwayat-cta-btn" onClick={() => onNavigate('gym-reservation')}>
            Buat Reservasi
          </button>
        </div>
        ...
        <>
          ...
          <div className="riwayat-summary" role="region" aria-label="Ringkasan reservasi">
            <div className="summary-chip">
              <div className="summary-chip-count" aria-label={`Total ${reservations.length} reservasi`}>
                {reservations.length}
              </div>
              <div className="summary-chip-label" aria-hidden="true">Total</div>
            </div>
            <div className="summary-chip">
              <div className="summary-chip-count blue" aria-label={`${counts.upcoming || 0} upcoming`}>
                {counts.upcoming || 0}
              </div>
              <div className="summary-chip-label" aria-hidden="true">Upcoming</div>
            </div>
            <div className="summary-chip">
              <div className="summary-chip-count red" aria-label={`${counts.berlangsung || 0} berlangsung`}>
                {counts.berlangsung || 0}
              </div>
              <div className="summary-chip-label" aria-hidden="true">Berlangsung</div>
            </div>
            <div className="summary-chip">
              <div className="summary-chip-count green" aria-label={`${counts.selesai || 0} selesai`}>
                {counts.selesai || 0}
              </div>
          ...
          <div className="riwayat-list" role="list" aria-label="Daftar reservasi">
            {filtered.length === 0 ? (
              <div className="riwayat-center" style={{ padding: '40px 24px' }}>
                ...
                <p className="riwayat-empty-desc">
                  Tidak ada reservasi dengan status "{STATUS_LABEL[filter]}"
                </p>
                <button className="riwayat-filter-reset" onClick={() => setFilter('all')}>
                  Tampilkan semua
                </button>
              </div>
            ) : (
              filtered.map((r) => {
                ...
                return (
                  <article
                    ...
                    key={r.id}
                    role="listitem"
                    aria-label={`Reservasi ${r.gym_name || 'Gym'} tanggal ${dt.getDate()} ${MONTHS[dt.getMonth()]} ${dt.getFullYear()}, status ${STATUS_LABEL[status]}`}
                  >
                    {/* Date box */}
                    ...
                    <div className="riwayat-info">
                      <div className="riwayat-weekday" aria-hidden="true">
                        {DAYS[dt.getDay()]} · {dt.getFullYear()}
                      </div>
                      <div className="riwayat-gymname">{r.gym_name || 'Gym'}</div>
                      <div className="riwayat-time" aria-label={`Pukul ${formatTime(r.start_time)} sampai ${formatTime(r.end_time)}`}>
                        {formatTime(r.start_time)} – {formatTime(r.end_time)}
                      </div>
                      {r.notes && (
                        <div className="riwayat-notes" title={r.notes}>{r.notes}</div>
                      )}
                      <span className={`riwayat-badge ${status}`} role="status">
                        {STATUS_LABEL[status]}
                      </span>
                    </div>
